...
6. `POST /lore/complete` with photo → verify image uploaded to S3 MinIO, Gemini Vision verifies, XP granted, appears in feed
...
10. Test multi-layer cache: cold start (no cache) → first request hits Gemini + writes to Redis + PostgreSQL → second request returns from Redis in <5ms
11. `GET /export/:tripId/pdf` → verify PDF URL returned from S3 MinIO
